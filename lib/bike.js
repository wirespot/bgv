...
var Bike = Class.create(Calculators, Renderers, {
    ...
    fixWheelBaseLevel: function () {
        ...
        var frontToRearDiffs = Util.coordinateDifferences(front[0], front[1], rear[0], rear[1]);
        ...
        for (prop in this.values) {
            ...
            var distance = Util.distanceBetweenPoints(rear[0], rear[1], part[0], part[1]);
            var partDiffs = Util.coordinateDifferences(rear[0], rear[1], part[0], part[1]);
     ...
     *   Receives the 'f' function of the definition as parameter.
     * @param skipTestFunc Function that tests whether this definition should
     *   be skipped. Receives as parameter the values entry with the same name
     *   as the definition.
     * @param prefix Informative prefix prepended to console messages.
     * @returns {boolean} Whether at least one dependency was met.
     */
    definitionsRun: function (definitions, depMetFunc, skipTestFunc, prefix) {
        // browse the definitions
        var anyDepsMet = false;
        for (prop in definitions) {
            ...
            for (i = 0; i < defCount; i++) {
                ...
                if (depsMet) {
                    ...
                    // remember that at deps were met at least one time
                    anyDepsMet = true;
                    //console.log(prefix, prop, 'deps met on def', i, this.values[prop]);
                    // no need to try the rest of the defs for this property,
                    // one is enough to achieve the purpose
                    break;
                }
                else {
        ...
        // report whether any deps were met
        return anyDepsMet;
    },
    ...
    renderBike: function () {
        ...
        this.definitionsRun(
            ...
            // TODO: add support for explicit skip markers
            function (x) {
                return false;
            },
            // log prefix
