var Bike = Class.create(Calculators, Renderers, {
    ...
    initialize: function (bikeDef, alignToBike, alignToPart) {
        ...
        var leveled = this.checkWheelBase();
        if (leveled) {
            this.Def['wheel_base'] = this.values.frontWheelHub[0] - this.values.rearWheelHub[0];
        }
        // alignToBike to other bike, if any
        if (typeof(alignToBike) === 'object') {
            this.alignToBikeVert(alignToBike, 'frontWheelHub');
            switch (alignToPart) {
                case 'front':
                    this.alignToBikeHor(alignToBike, 'frontWheelHub');
                    break;
                case 'rear':
                    this.alignToBikeHor(alignToBike, 'rearWheelHub');
                    break;
                default:
                    this.alignToBikeHor(alignToBike, 'bottomBracket');
                    break;
            }
        }
        else {
    ...
    checkWheelBase: function () {
        ...
        console.log(this.Def.name + ': Rear wheel not level with front by ' + diff[1] + 'px, adjusting.');
...
console.log(prop, this.values[prop]);
            }
        }
...
console.log(minX, maxX, minY, maxY, this.values.centerSpot);
    },
    ...
    alignToBikeHor: function(otherBike, targetPart) {
        this.alignToBike(otherBike, targetPart, 0);
    },

    alignToBikeVert: function(otherBike, targetPart) {
        this.alignToBike(otherBike, targetPart, 1);
    },

    alignToBike: function(otherBike, targetPart, index) {
        // establish delta
