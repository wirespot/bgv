...
var Bike = Class.create(Calculators, Renderers, {
    ...
    checkWheelBase: function () {
        ...
        var diff = Util.coordinateDifferences(f[0], f[1], r[0], r[1]);
        ...
        for (prop in this.values) {
            ...
            var distance = Util.distanceBetweenPoints(r[0], r[1], p[0], p[1]);
            var pDiff = Util.coordinateDifferences(r[0], r[1], p[0], p[1]);
        ...
        console.log(this.Def.name + ': Rear wheel not level with front by ' + diff[1] + 'px'
            + ', adjusted angle by ' + Util.radToDeg(rotAngle) + ' deg'
