...
var Bike = Class.create(Calculators, Correctors, Renderers, {
    ...
    initialize: function (bikeDef, alignToBike, alignToPart) {
        ...
        this.renderBike();
        // all done
        Log.print(this.Def.name + ': bike done.');
    },
    ...
    fixWheelBaseLevel: function () {
        ...
        Log.print(this.Def.name + ': wheels not level, bike tilted by '
        ...
        for (prop in definitions) {
            ...
            for (i = 0; i < defCount; i++) {
                ...
                if ('undefined' == typeof(defs[i]['dependencies'])) {
                    Log.print(logPrefix + prop + ' is missing the dependencies array!');
                    continue;
                }
                if ('function' != typeof(defs[i]['f'])) {
                    Log.print(logPrefix + prop + ' is missing the "f" function!');
                    continue;
                }
                // init log
                ...
                if (depsMet) {
                    ...
                    if ((defDoneFunc.bind(this))(prop)) {
                        ...
                        Log.print(logMessage + ' done; skipping rest.');
                        break;
                    }
                    else {
                        Log.print(logMessage + ' NOT done.');
                    }
                }
                else {
                    Log.print(logMessage + ' deps NOT met.');
                }
            }
        }
    },
            ...
            function (prop, f) {
                ...
                if (null !== val && 'undefined' != typeof(val)) {
                    if ('undefined' == typeof(this.Def[prop])) {
                        Log.print(this.Def.name + ': corr: ' + prop
