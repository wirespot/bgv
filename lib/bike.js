 ...
 * It mixes in various definition collections.
 */
...
var Bike = Class.create(Calculators, Correctors, Renderers, Labels, {
    ...
    initialize: function (bikeDef, alignToBike, alignToPart) {
        ...
        // draw bike
        this.renderBike();
        if (typeof(this.Def['labels']) == 'boolean' && this.Def.labels
            || typeof(this.Def['labels']) != 'boolean' && typeof(alignToBike) !== 'object'
        ) {
            this.printLabels();
        }
        // all done
        Log.info(this.Def.name + ': bike done.');
    },
    ...
    fixWheelBaseLevel: function () {
        ...
        for (var prop in this.values) {
            ...
            // reference)
            if (!this.values.hasOwnProperty(prop)) continue;
            if (!this.isCoordinate(this.values[prop])
                || prop == 'rearContactPatch') continue;
        ...
        Log.info(this.Def.name + ': wheels not level, bike tilted by '
        ...
        for (var prop in definitions) {
            ...
            for (var i = 0; i < defCount; i++) {
                ...
                if ('undefined' == typeof(defs[i]['dependencies'])) {
                    Log.warn(logPrefix + prop + ' is missing the dependencies array!');
                    continue;
                }
                if ('function' != typeof(defs[i]['f'])) {
                    Log.warn(logPrefix + prop + ' is missing the "f" function!');
                    continue;
                }
                // init log
                ...
                if (depsMet) {
                    ...
                    (depMetFunc.bind(this))(prop, defs[i]);
                    ...
                    if ((defDoneFunc.bind(this))(prop)) {
                        ...
                        Log.debug(logMessage + ' done; skipping rest.');
                        break;
                    }
                    else {
                        Log.debug(logMessage + ' NOT done.');
                    }
                }
                else {
                    Log.debug(logMessage + ' deps NOT met.');
                }
            }
        }
    },
    ...
    renderBike: function () {
        ...
        this.definitionsRun(
            ...
            function (prop, def) { (def.f.bind(this))(); },
    ...
    printLabels: function () {
        ...
        Canvas.context.fillStyle = this.Def.color;
        Canvas.context.strokeStyle = this.Def.color;
        // do a single run through all the labels
        this.definitionsRun(
            this.labels,
            // there is no skip test, all labels are to be printed
            function () { return false; },
            // use the 'f' function to draw
            function (prop, def) { (def.f.bind(this))(); },
            // definition is always done
     ...
     * @returns {int} How many definitions were corrected.
     */
    correctBikeDefs: function () {
        ...
        this.definitionsRun(
            ...
            function (prop, def) {
                var val = (def.f.bind(this))();
                // we only care if the function produced a valid value
                if (null !== val && 'undefined' != typeof(val)) {
                    // if there was no value before use this one
                    if ('undefined' == typeof(this.Def[prop])) {
                        this.Def[prop] = val;
                        newDefs++;
                        Log.info(this.Def.name + ': corr: ' + prop
                            + ' is missing, using ' + val + '.');
                    }
                    // if there was a value already, replace it only if
                    // the definition doesn't have the fixed marker set
                    else if (val != this.Def[prop]
                        && (typeof(def['fixed']) != 'boolean' || !def['fixed'])
                    ) {
                        this.Def[prop] = val;
                        Log.info(this.Def.name + ': corr: ' + prop
    ...
    /**
     * Calculate values and defs repeatedly until neither produces new stuff.
     */
    cyclicCalculations: function () {
        var that = this;
        var first = true;
        var doVals = function () {
            var foo = (that.calculateValues.bind(that))();
            if (foo || first) {
                first = false;
                doDefs();
            }
        ...
        var doDefs = function () {
            var foo = (that.correctBikeDefs.bind(that))();
            if (foo) doVals();
        };
        doVals();
    },
