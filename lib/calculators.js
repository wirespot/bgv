 ...
 * covered. The calculators are run once before correctors and once after, if
 * any corrects changed any definitions; and before renderers and labelers run.
 *
...
var Calculators = {
    ...
    calculators: {
        ...
        headTop: [{
            ...
            f: function () {
                var downAngle = Util.triangleAngleBySides(
                    this.Def.seat_tube, this.Def.frame_htt_bb,
                    this.Def.frame_htt_st
                );
                var angle = Math.PI + Util.degToRad(this.Def.seat_angle) + downAngle;
                return Util.findPoint(
                    this.values.bottomBracket, Util.zoomApply(this.Def.frame_htt_bb), angle);
            }
        }],
        ttlSpot: [{
            dependencies: [
        ...
        headBottom: [{
            ...
            f: function () {
                ...
                var downAngle = Util.triangleAngleBySides(
                    this.Def.seat_tube, this.Def.frame_htb_bb,
                    this.Def.frame_htb_st
                );
                var angle = Math.PI + Util.degToRad(this.Def.seat_angle) + downAngle;
                return Util.findPoint(
                    this.values.bottomBracket, Util.zoomApply(this.Def.frame_htb_bb), angle);
            }
        }, {
            dependencies: [
                ['values', 'headTop'],
        ...
        forkTrailSpot: [{
            ...
            f: function () {
                return [
                    this.values.headBottom[0]
                        + (this.values.frontContactPatch[1] - this.values.headBottom[1])
                            / Math.tan(Util.degToRad(this.Def.head_angle)),
                    this.values.frontContactPatch[1]
                ];
            }
        }],
        forkStraight: [{
        ...
        frontWheelHub: [{
            ...
            f: function () {
                var alpha = Util.degToRad(this.Def.head_angle);
                var b = Util.zoomApply(this.Def.fork_offset) / Math.sin(alpha);
                var H = Math.abs(this.values.rearWheelHub[1] - this.values.headTop[1]);
                var A = H / Math.tan(alpha);

                //var a = b * Math.sin(Math.PI - alpha);
console.log(H,A+b);
                return [this.values.headTop[0] + A + b, this.values.rearWheelHub[1]];
                //return Util.zoomApply(B + a);
            }
        }],
        frontContactPatch: [{
            dependencies: [
